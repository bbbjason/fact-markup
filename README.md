...
GitHub Actions automation for the Codex `fact-markup` workflow. It fetches or reads an article, removes obvious boilerplate, normalizes the article to line-based text units, asks a Mistral model to classify each unit as `FACT` or `NONFACT`, and writes two sibling outputs:
...
2. Add an Actions secret named `MISTRAL_API_KEY`.
3. Optional: add an Actions variable named `MISTRAL_MODEL` if you want to override the default `mistral-medium-latest`.
